...
class Solution {
    public String solution(String[] cards1, String[] cards2, String[] goal) {
        ...
        while(go<goal.length) {
            if(ca1 <cards1.length && goal[go].equals(cards1[ca1])) {
                go++;
                ca1++;
            }else if(ca2 <cards2.length && goal[go].equals(cards2[ca2])) {
                go++;
                ca2++;
            }else break;
        }

        return go == goal.length ? "Yes" : "No";
    }
